...
using StreamJsonRpc.Sample.Client;
using Xunit;
using Xunit.Abstractions;

public class UnbatchingWebSocketMessageHandlerTests : TestBase
{
    ...
    private UnbatchingWebSocketMessageHandler handler;
    private JsonMessageFormatter formatter = new JsonMessageFormatter();

    public UnbatchingWebSocketMessageHandlerTests(ITestOutputHelper logger)
        : base(logger)
    {
        this.socket = new MockWebSocket();
        this.handler = new UnbatchingWebSocketMessageHandler(this.socket, this.formatter, BufferSize);
    }
    ...
    {
        Assert.Throws<ArgumentNullException>(() => new UnbatchingWebSocketMessageHandler(null!));
        Assert.Throws<ArgumentOutOfRangeException>(() => new UnbatchingWebSocketMessageHandler(new MockWebSocket(), new JsonMessageFormatter(), 0));
        Assert.Throws<ArgumentOutOfRangeException>(() => new UnbatchingWebSocketMessageHandler(new MockWebSocket(), new JsonMessageFormatter(), -1));
        new UnbatchingWebSocketMessageHandler(new MockWebSocket(), new JsonMessageFormatter(), 1);
        new UnbatchingWebSocketMessageHandler(new MockWebSocket(), new JsonMessageFormatter(), 1 * 1024 * 1024);
    }
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    [Fact]
    public async Task BatchedMessages()
    {
        this.Send(new dynamic[]
        {
            new
            {
                jsonrpc = "2.0",
        ...
        this.socket.EnqueueRead(Array.Empty<byte>());

        var results = new List<JsonRpcMessage>();
        JsonRpcMessage? result;
        while ((result = await this.handler.ReadAsync(this.TimeoutToken)) is object)
        {
            results.Add(result);
        }

        Assert.Equal(5, results.Count);
        Assert.Equal(1, ((JsonRpcResult)results[0]).RequestId.Number);
        Assert.Equal(2, ((JsonRpcRequest)results[1]).RequestId.Number);
        Assert.Equal("highfive", ((JsonRpcRequest)results[2]).Method);
        Assert.Equal("bye", ((JsonRpcRequest)results[3]).Method);
        Assert.Equal("single", ((JsonRpcRequest)results[4]).Method);
    }
    ...
    {
        this.handler = new UnbatchingWebSocketMessageHandler(this.socket, new JsonMessageFormatter(), 2);
    ...
    private async Task<(JsonRpc JsonRpc, WebSocket WebSocket)> EstablishWebSocket()
    {
        ...
        var rpc = new JsonRpc(new UnbatchingWebSocketMessageHandler(webSocket));
        rpc.StartListening();
    ...
    private void Send(dynamic message)
    {
        ...
        this.socket.EnqueueRead(Encoding.UTF8.GetBytes(json));
    }
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env)
        {
            app.Use(async (context, next) =>
            {
                if (context.WebSockets.IsWebSocketRequest)
                {
                    ...
                    using (var rpc = new JsonRpc(new UnbatchingWebSocketMessageHandler(webSocket), new EchoServer(webSocket)))
                    {
                        rpc.StartListening();
